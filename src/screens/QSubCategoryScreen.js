...
export default function ({ navigation }) {
  ...
  return (
    <Layout>
      <TopNav
        middleContent="Quick Search By Sub Category"
        leftContent={
          <Ionicons
            name="chevron-back"
            size={20}
     ...
     <View style={styles.formContent}>
      ...
      {isLoading ? (
        ...
        <FlatList
          ...
          renderItem={ ({ item }) => { return ( <TouchableOpacity  onPress={() =>navigation.navigate('QuickProductListScreen')}>
              <View style={styles.notificationBox}>
                <Image style={styles.image}
                  source={{uri: item.icon}}/>
                
                <Text style={styles.description}>{item.protitle}</Text>
              </View>
          </TouchableOpacity>
           )} }
        />
